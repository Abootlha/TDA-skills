...
export default function CreateNVQPage() {
    ...
    const [formData, setFormData] = useState({
        name: "",
        slug: "",
        category: "nvq",      // Default to "nvq" category
        sub_category: "level-3", // Default Level 3
        type: "nvq",          // Locked to "nvq" type
        description: "",
        price: "",
        sale_price: "",
        deposit: "",
        ...
        quick_stats: {
            duration: "8 - 14 Weeks", // Default NVQ duration
            delivery: "Online Portfolio",
            nextDate: "Flexible Start",
            grant: "N/A"
        },
        included: ["Assigned Assessor", "e-Portfolio Access", "Regulated Certificate"] as string[],
        overview: {
            whatIsIt: [""],
            whoShouldAttend: "",
            certification: ""
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log("Submitting NVQ Payload to Backend:", payload);
            ...
            console.error("Error creating NVQ qualification:", error.response?.data || error);
            alert("Failed to create qualification: " + (error.response?.data?.error || error.message));
        ...
        <div className="max-w-[1200px] mx-auto pb-20 animate-in fade-in zoom-in duration-300">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <h1 className="text-3xl font-black text-[#001430] tracking-tight">Create NVQ Qualification</h1>
                </div>
                <div className="flex gap-3">
                    <button onClick={handleSave} disabled={isSubmitting} className="flex items-center gap-2 px-6 py-2.5 bg-[#FFB800] text-[#001430] rounded-lg text-sm font-bold hover:bg-[#e6a600] transition-colors shadow-sm disabled:opacity-50">
                        <Save size={18} />
                        {isSubmitting ? "Saving..." : "Publish Qualification"}
                    </button>
                </div>
            </div>
