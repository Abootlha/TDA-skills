...
export function EnquiriesTable() {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500 font-bold">Loading enquiries...</div>;
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-50">
            {enquiries.length === 0 ? (
              ...
              <tr key={enq.id} className="hover:bg-gray-50/80 transition-colors group">
                <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                  {formatDate(enq.created_at)}
                </td>
                <td className="px-6 py-4">
                  <div className="font-bold text-[#001430]">{enq.full_name}</div>
                  <div className="text-xs text-gray-500 mt-0.5">{enq.email} • {enq.phone_number}</div>
                </td>
                <td className="px-6 py-4">
                  <span className="text-sm font-medium text-gray-600 bg-gray-100 px-2.5 py-1 rounded-md">{enq.enquiry_type || 'General'}</span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {enq.status !== 'resolved' && (
                      <button 
                        onClick={() => updateStatus(enq.id, 'resolved')}
      ...
      {viewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 relative animate-in fade-in zoom-in duration-200">
            <h3 className="text-xl font-bold text-[#001430] mb-4">Enquiry Details</h3>
